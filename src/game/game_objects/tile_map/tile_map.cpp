  ...
  for (size_t i = 0;
    ...
    if (tileAttributesData.tileData[i].display.type ==
      ...
      images[i] = IMG_LoadTexture(
          renderer, tileAttributesData.tileData[i].display.data.imageFile);
  ...
  for (size_t i = 0; i < NUMBER_OF_TILES; i++) {
    if (tileMap[i] == TILE_ROTATION) {
      ...
      if (tiles.right != NULL && tiles.bottomRight != NULL &&
          ...
          tiles.left != NULL && tiles.topLeft != NULL && tiles.top != NULL &&
          tiles.topRight != NULL) {
        TILE_TYPE tileValue[8] = {
            *tiles.right, *tiles.bottomRight, *tiles.bottom, *tiles.bottomLeft,
            *tiles.left,  *tiles.topLeft,     *tiles.top,    *tiles.topRight};
    ...
    if (tileMap[i] == TILE_ENEMY_SPAWNER && tileOutlines == false &&
      ...
      enemy->enemy = {position -
                      (Vector2){(double)tileSize / 2, (double)tileSize / 2}};
    }
  }
}
...
void TileMap::draw(void) {
  for (size_t i = 0; i < NUMBER_OF_TILES; i++) {
    ...
    if (rectangles[i].x > windowWidth) {
      ...
      double addedOn =
          (cameraPosition.x - position.x - windowWidth / 2) / (int)tileSize - 1;
      if (addedOn > 0)
        i += (size_t)addedOn;
      continue;
    }
    ...
    if (tileAttributes.display.type == TileDisplayData::TILE_DISPLAY_COLOUR) {
      SDL_SetRenderDrawColour(renderer, tileAttributes.display.data.colour.r,
...
void TileMap::drawShadows(void) {
  for (size_t i = 0; i < NUMBER_OF_TILES; i++) {
    TILE_MAP_RECTANGLES_POSITION
    rectangles[i].x = (int)round((double)rectangles[i].x - cameraPosition.x +
                                 (double)windowWidth / 2);
    rectangles[i].y = (int)round((double)rectangles[i].y - cameraPosition.y +
    ...
    if (tileMap[i] == TILE_AIR || tileMap[i] == TILE_NONE ||
        (tileAttributesData.tileData[tileMap[i]].display.visibleInEditorOnly &&
         !tileOutlines))
      continue;

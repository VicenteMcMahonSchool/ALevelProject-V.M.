...
TileMap::TileMap(TILE_MAP_CONSTRUCTOR_ARGUMENTS)
    : GeneralGameObject(position), tileSize(tileSize) {
#ifdef ONE_TILE_MAP
    ...
    for (size_t i = 0;
         i < sizeof(tileAttributesData.tileData) / sizeof(TileAttributes); i++)
        if (tileAttributesData.tileData[i].display.type ==
            TileDisplayData::TILE_DISPLAY_IMAGE)
            images[i] = IMG_LoadTexture(
                renderer,
                tileAttributesData.tileData[i].display.data.imageFile);
    ...
    for (size_t i = 0; i < NUMBER_OF_TILES; i++) {
        if (tileMap[i] == TILE_ROTATION) {
            TileCentres tileCentres =
                getTileCentresAroundPositionOfTile(tileMap + i);
            TilesAroundTile tiles = getTilesAroundIndex(i);
            if (tiles.right != NULL && tiles.bottomRight != NULL &&
                tiles.bottom != NULL && tiles.bottomLeft != NULL &&
                tiles.left != NULL && tiles.topLeft != NULL &&
                tiles.top != NULL && tiles.topRight != NULL) {
                TILE_TYPE tileValue[8] = {*tiles.right,  *tiles.bottomRight,
                                          *tiles.bottom, *tiles.bottomLeft,
                                          *tiles.left,   *tiles.topLeft,
                                          *tiles.top,    *tiles.topRight};
        ...
        if (tileMap[i] == TILE_ENEMY_SPAWNER && tileOutlines == false &&
            numberOfTicks % 3 == 0) {
            Vector2 position = getCentrePositionOfTile(tileMap + i);
            GameObjectUnion *enemy = gameObjects.add(ENEMY);
            enemy->enemy = {position - (Vector2){(double)tileSize / 2,
                                                 (double)tileSize / 2}};
        }
    }
}
...
void TileMap::draw(void) {
    for (size_t i = 0; i < NUMBER_OF_TILES; i++) {
        TILE_MAP_RECTANGLES_POSITION
        rectangles[i].x -= cameraPosition.x - (float)windowWidth / 2;
        rectangles[i].y -= cameraPosition.y - (float)windowHeight / 2;
        if (rectangles[i].x > windowWidth) {
            i = (i / WIDTH_OF_TILE_MAP + 1) * WIDTH_OF_TILE_MAP - 1;
            continue;
        } else if (rectangles[i].x + (int)tileSize < 0) {
            double addedOn =
                (cameraPosition.x - position.x - (float)windowWidth / 2) /
                    (int)tileSize -
                1;
            if (addedOn > 0)
                i += (size_t)addedOn;
        ...
        if (rectangles[i].y > windowHeight)
            ...
            long int addedOn = ((long int)(cameraPosition.y - position.y -
                                           (float)windowHeight / 2 + 1) /
                                (long int)tileSize) *
                                   WIDTH_OF_TILE_MAP -
                               1;
            if (addedOn > 0)
        ...
        if (tileMap[i] == TILE_AIR || tileMap[i] == TILE_NONE ||
            tileAttributes.display.visibleInEditorOnly && !tileOutlines)
            goto doNotFill;
        if (tileAttributes.display.type ==
            TileDisplayData::TILE_DISPLAY_COLOUR) {
            SDL_SetRenderDrawColour(renderer,
                                    tileAttributes.display.data.colour.r,
                                    tileAttributes.display.data.colour.g,
                                    tileAttributes.display.data.colour.b,
                                    tileAttributes.display.data.colour.a);
            SDL_RenderFillRect(renderer,
                               rectangles + i); // Fills the rectangle.
        } else if (tileAttributes.display.type ==
                   TileDisplayData::TILE_DISPLAY_IMAGE)
...
void TileMap::drawShadows(void) {
    for (size_t i = 0; i < NUMBER_OF_TILES; i++) {
        TILE_MAP_RECTANGLES_POSITION
        rectangles[i].x =
            (int)round((double)rectangles[i].x - cameraPosition.x +
                       (double)windowWidth / 2);
        rectangles[i].y =
            (int)round((double)rectangles[i].y - cameraPosition.y +
        ...
        if (tileMap[i] == TILE_AIR || tileMap[i] == TILE_NONE ||
            (tileAttributesData.tileData[tileMap[i]]
                 .display.visibleInEditorOnly &&
             !tileOutlines))
            continue;
        SDL_Rect shadowRectangle{rectangles[i].x, rectangles[i].y,
                                 rectangles[i].w + SHADOW_DISTANCE_X,
                                 rectangles[i].h + SHADOW_DISTANCE_Y};
...
size_t TileMap::getTileIndex(const TILE_TYPE *tile) { return tile - tileMap; }
...
void TileMap::setTilesAroundPosition(Vector2 position, TILE_TYPE tileType,
                                     size_t distance) {
    if (distance == 1)
    ...
    size_t topCornerDifference =
        distance / 2 +
        WIDTH_OF_TILE_MAP * (distance / 2); // Avoids integer underflow.
    size_t topIndex = 0;
...
size_t TileMap::getIndexFromPosition(Vector2 position) {
    return (size_t)round(position.y) / tileSize * WIDTH_OF_TILE_MAP +
           (size_t)round(position.x) / tileSize;
}
...
TilesAroundTile TileMap::getTilesAroundIndex(size_t index) {
    ...
    if (index - WIDTH_OF_TILE_MAP <= NUMBER_OF_TILES &&
        index - WIDTH_OF_TILE_MAP >= 0)
    ...
    if (index - WIDTH_OF_TILE_MAP - 1 <= NUMBER_OF_TILES &&
        index - WIDTH_OF_TILE_MAP - 1 >= 0)
...
Vector2 TileMap::getCentrePositionOfTile(const TILE_TYPE *tile) {
    ...
    const size_t tileYIndex = tileIndex / WIDTH_OF_TILE_MAP;
    return {(tileIndex % WIDTH_OF_TILE_MAP) * tileSize + (double)tileSize / 2,
            tileYIndex * (tileSize) + (double)tileSize / 2};
}
...
TileCentres TileMap::getTileCentresAroundPositionOfTile(const TILE_TYPE *tile) {
    ...
    output.topLeft =
        output.centre + Vector2{-(double)tileSize, -(double)tileSize};
...
TileAttributes &TileMap::getTileAttributes(const TILE_TYPE tile) {
    if (tile >= TILE_TYPE::TILE_NORMAL_MAXIMUM_VALUE)
        return tileAttributesData.tileData[TILE_AIR];

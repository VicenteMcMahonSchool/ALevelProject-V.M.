...
#define TILE_MAP_RECTANGLES_POSITION                                           \
    const unsigned int rectangleYIndex = i / WIDTH_OF_TILE_MAP;                \
    rectangles[i].x = (i % WIDTH_OF_TILE_MAP) * tileSize - position.x;         \
    rectangles[i].y = rectangleYIndex * tileSize - position.y;
...
class TileMap : public GeneralGameObject {
  ...
  public:
    ...
    void setTilesAroundPosition(Vector2 position, TILE_TYPE tileType,
                                size_t distance);

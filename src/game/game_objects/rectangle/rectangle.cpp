...
{
    rectangle.x = (int)round(position.x - (cameraPosition.x - windowWidth / 2));
    rectangle.y = (int)round(position.y - (cameraPosition.y - windowHeight / 2));
    // Fills the rectangle.
...
{
    shadowRectangle.x = (int)round(position.x - (cameraPosition.x - (double)windowWidth / 2)) /* + SHADOW_DISTANCE_X */;
